...
const mockLeaderboard = [
  ...
  { rank: 6, name: 'Anna Taylor', points: 2700, isCurrentUser: false, avatarColor: 'bg-[#77d6bd]' },
  { rank: 7, name: 'Raj Patel', points: 2600, isCurrentUser: false, avatarColor: 'bg-[#fed282]' },
  { rank: 8, name: 'Sophia Lee', points: 2500, isCurrentUser: false, avatarColor: 'bg-[#b587fb]' },
  { rank: 42, name: 'Alice Student', points: 1240, isCurrentUser: true, avatarColor: 'bg-black' },
];
...
export default function Leaderboard() {
  ...
  return (
    <motion.div 
      ...
      className="h-full bg-gradient-insights flex flex-col relative"
    >
      {/* Scrollable Area */}
        ...
        <div className="px-6 flex flex-col gap-3 pb-6">
          <div className="bg-white rounded-[24px] p-2 shadow-soft flex flex-col gap-1">
            {mockLeaderboard.map((u) => (
              <div 
                key={u.rank} 
                className={`flex items-center justify-between p-3 rounded-[16px] ${u.isCurrentUser ? 'bg-gray-100' : 'bg-transparent text-black'}`}
              >
                <div className="flex items-center gap-3">
                  <span className={`w-6 text-center font-bold text-[14px] text-gray-500`}>
                    {u.rank}
                  </span>
      ...
      {/* Pinned Current User (Fixed at bottom above nav bar) */}
      <div className="absolute bottom-[90px] inset-x-6 z-20">
        <div className="bg-[#0E0E0E] text-white rounded-[20px] p-4 flex items-center justify-between shadow-2xl border border-gray-800">
           ...
           <span className="font-bold text-[14px] text-[#fed282]">{currentUser?.points || user?.totalPoints} pts</span>
        </div>
      </div>
    </motion.div>
  );
